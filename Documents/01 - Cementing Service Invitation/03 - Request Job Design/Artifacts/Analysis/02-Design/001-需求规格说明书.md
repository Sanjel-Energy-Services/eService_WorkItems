...
### 1.4 Phase 1 范围裁剪说明（相对 FRD1）
依据《001-Program_Request_Management_FRD 1.md》与《002-Program request Management tool Phase 1.md》，Phase 1 对范围进行如下裁剪与聚焦：
- 保留：请求接收、Program ID 生成、公司-工程师自动分配、基于到期日的自动优先级、矩阵色审查/审批流程、状态时间线审计、文档管理、活动日志、搜索与可视化追踪；
- 聚焦：工程侧自动分配与通知、评审与审批分离执行（触发路径不同但界面形态类似）；
- 扩展：支持多作业类型（Multi-Job）在审批路径上的区分；MVP 允许最高权限一次性整单批准；
- 新增（相对FRD1的明确化）：销售自动分配（公司-销售映射）、邮件与应用内通知模板；
- 暂不纳入：与外部服务系统的强耦合集成；保留可选的文档外链（如 SharePoint/eService）。
...
**需求标识**: REQ-REQ-001
**需求名称**: 程序请求创建与自动分配
**业务描述**: 系统必须允许销售或调度人员通过电子邮件或门户网站提交新的程序请求。系统在收到请求后应自动生成唯一 Program ID，并根据预设的“公司-工程师”映射表自动将任务分配给相应的工程师。
...
sequenceDiagram
    ...
    S->>SYS: submitRequest(requesterInfo, client, requestType, ...)
    activate SYS
    SYS->>SYS: validateRequiredFields()
    alt 验证失败
        SYS-->>S: showValidationError()
    else 验证成功
        SYS->>SYS: generateProgramID()
        SYS->>EM: findAssignedEngineer(clientCompany)
        activate EM
        EM-->>SYS: assignedEngineerInfo
        deactivate EM

        alt 找到分配工程师
            SYS->>E: assignRequest(programRequest)
            activate E
            E-->>SYS: assignmentAcknowledged
            deactivate E
            SYS->>SYS: updateRequestStatus("Assigned")
        else 未找到分配工程师
            SYS->>M: assignRequest(programRequest)
            activate M
            M-->>SYS: assignmentAcknowledged
            deactivate M
            SYS->>SYS: updateRequestStatus("Assigned to Manager")
        end

        SYS-->>S: showSuccessMessage(ProgramID)
    end
    deactivate SYS
```
...
5. 评审（Review）与审批（Approval）为不同触发过程，界面形态可相似但动作与结果不同；两者的通知与状态更新分离管理（引用 Phase 1）。
...
classDiagram
    ...
    ProgramRequest "1" --> "1" CompanySalesMapping : maps_to_sales
```
...
规则实现：系统必须自动计算优先级（Mandatory），由 `calculatePriority()` 根据 `dueDate - currentTime` 动态评估；界面允许颜色提示随到期临近而变化；若人工调整（仅在特殊场景），需记录变更原因与审计条目。
...
#### 3.13.2 工作流与文档外部集成（Power Automate / SharePoint）
如采用外部工作流与文档平台，支持以下集成点：
- 工作流触发与路由：通过 Power Automate 监听 PRM 事件（Webhook）触发审批/通知流程；回写审批结果与时间戳至 PRM。
- 文档管理：在 SharePoint 创建/更新程序文档库条目，保持版本与访问控制；PRM 保存关联引用（链接/ID）。
- 安全与合规：使用应用注册/凭据进行 API 访问，限制最小权限；所有调用记录入审计日志。
- 失败处理：调用失败重试与告警；人工回退方案（例如在 PRM 端重发事件）。

#### 3.13.1 销售自动分配通知（Phase 1 新增）
基于公司-销售映射进行自动分配，并发送邮件通知模板：
- 主题：Your program is approved and available for pricing
- 正文示例：
    Hello [Name]
    Your program is approved and available for pricing
    Thank you
系统需记录通知发送与阅读回执（可选）以支持审计。
...
本说明书的结构与模型均严格引用并体现《G 信息化建模过程详解-5 需求获取：从客户描述到规格说明》中所述方法论，确保需求从客户原始描述到可执行规格的可追溯性与一致性。
