...
### 1.2 范围
本系统将自动化并标准化从接收、分配、评审、批准到跟踪的完整程序请求管理生命周期。系统边界包括与FreshService（或新定制应用）的集成，涵盖销售、调度、工程师、审批人及现场团队等多个用户角色。
...
sequenceDiagram
    ...
    participant FS as FreshService System
...
- 邮件转发地址（FreshService）：`help_6@sanjel.freshservice.com`
- 应用内通知：支持Web与Mobile推送（含深链接至请求详情）。
- API集成：
    - FreshService Tickets API：创建/更新请求、附件处理、状态同步
    - Users/Groups API：角色与分配关系同步
    - Webhook：状态变更、审批决定、优先级变更回调
- 失败重试：通知/同步失败采用指数退避（最多3次），记录失败原因与payload用于审计。
...
3.  **集成性**:
    - 必须通过API与FreshService（或定制应用）无缝集成
    - 支持与外部文档系统（如SharePoint, eService）链接
