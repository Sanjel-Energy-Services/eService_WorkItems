...
namespace Sanjel.PrintingService.Reports
{
    public class SectionBase : ISection
    {
        ...
        {
            return PriceBookUtilities.GetPropertyValueByPath(Model.DataSet, propertyPath);
        }

        public void SetupDataSource(ReportDataSource reportModel)
        {
            Model = new DataModel(reportModel);
        }
        ...
        public void SetCurrentReportDataSource(ReportDataSource dataSource)
        {
            _model.SetCurrentReportModel(dataSource);
        }
    }
}
