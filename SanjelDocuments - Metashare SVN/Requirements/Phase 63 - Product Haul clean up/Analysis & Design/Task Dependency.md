...
```mermaid
graph LR
A[Sales submits program request]
B[Engineer creates Job Design]
C[Dispatch creates Call Sheet]
D[Supervisor creates Job Package]
E[Supervisor submites Job Package]
F[Logistics Dispatch creates Blend Request]
G[Bulk Plant Operator mixes blend]
H[Bulk Plant Operator loads blend to bin]
I[Bulk Plant Operator loads blend to bulker]
J[Bulker Driver ships blend to destination]
K[Logistics Dispatch Create Haul Schedule]
```
