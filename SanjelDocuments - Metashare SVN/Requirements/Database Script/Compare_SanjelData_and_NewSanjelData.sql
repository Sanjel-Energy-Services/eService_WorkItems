...
(select schema_name(tab.schema_id) + '.' + tab.name as table_name, 
   ...
   from [SanjelData].sys.tables as tab
        inner join [SanjelData].sys.columns as col
            on tab.object_id = col.object_id) db1
full outer join
(select schema_name(tab.schema_id) + '.' + tab.name as table_name, 
       col.name as column_name
   from [SanjelData_1].sys.tables as tab
        inner join [SanjelData_1].sys.columns as col
