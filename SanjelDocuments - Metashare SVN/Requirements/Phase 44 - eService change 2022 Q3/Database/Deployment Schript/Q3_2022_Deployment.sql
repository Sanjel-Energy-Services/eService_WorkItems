...
SELECT  [TYPE_INDEX] [id],1 [version],15112 [modified_user_id],'Awang' [modified_user_name],GetDate() [modified_datetime],3 [operation_type], GetDate() [effective_start_datetime],'9999-12-31 00:00:00.0000000' [effective_end_datetime], 0 [entity_status],0 [owner_id], [DESCRIPTION]  [name], [DESCRIPTION]  [description] FROM [eService6UAT].[dbo].[TYPE_VALUES_ConvertedToView] WHERE [TYPE_ID] = N'116' ORDER BY [TYPE_INDEX]
GO
...
SELECT  [TYPE_INDEX] [id],1 [version],15112 [modified_user_id],'Awang' [modified_user_name],GetDate() [modified_datetime],3 [operation_type], GetDate() [effective_start_datetime],'9999-12-31 00:00:00.0000000' [effective_end_datetime], 0 [entity_status],0 [owner_id], [DESCRIPTION]  [name], [DESCRIPTION]  [description] FROM [eService6UAT].[dbo].[TYPE_VALUES_ConvertedToView] WHERE [TYPE_ID] = N'117' ORDER BY [TYPE_INDEX]
GO
...
(
    select
        ...
        --, owner_id
    from 
        (select ROW_NUMBER() OVER (Partition By Id ORDER BY Version desc) as rowNum, * from SanjelDataUAT.dbo.PlugLoadingHeadSpecialty) a
    where 
        rowNum = 1
)
,mdm_i as 
(
    select
        ...
        --, owner_id
    from 
        (select ROW_NUMBER() OVER (Partition By Id ORDER BY Version desc) as rowNum, * from SanjelDataUAT.dbo.PlugManufacture) a
    where 
        rowNum = 1
)
,mdm_h as 
(
    select
        ...
        --, owner_id
    from 
        (select ROW_NUMBER() OVER (Partition By Id ORDER BY Version desc) as rowNum, * from SanjelDataUAT.dbo.PlugLoadingHeadManufacture) a
    where 
        rowNum = 1
)
,mdm_g as 
(
    select
        ...
        --, owner_id
    from 
        (select ROW_NUMBER() OVER (Partition By Id ORDER BY Version desc) as rowNum, * from SanjelDataUAT.dbo.QualityObservationType) a
    where 
        rowNum = 1
)
,mdm_f as 
(
    select
        ...
        --, owner_id
    from 
        (select ROW_NUMBER() OVER (Partition By Id ORDER BY Version desc) as rowNum, * from SanjelDataUAT.dbo.PotentialSeverity) a
    where 
        rowNum = 1
)
,mdm_e as 
(
    select
        ...
        --, owner_id
    from 
        (select ROW_NUMBER() OVER (Partition By Id ORDER BY Version desc) as rowNum, * from SanjelDataUAT.dbo.PerformanceMetric) a
    where 
        rowNum = 1
)
,es_e_extra as 
...
(
    select
        ...
        --, owner_id
    from 
        (select ROW_NUMBER() OVER (Partition By Id ORDER BY Version desc) as rowNum, * from SanjelDataUAT.dbo.IncidentType) a
    where 
        rowNum = 1
)
,es_d_extra as 
...
(
    select
        ...
        --, owner_id
    from 
        (select ROW_NUMBER() OVER (Partition By Id ORDER BY Version desc) as rowNum, * from SanjelDataUAT.dbo.SeverityMatrixtype) a
    where 
        rowNum = 1
)
,es_c_extra as 
...
(
    select
        ...
        --, owner_id
    from 
        (select ROW_NUMBER() OVER (Partition By Id ORDER BY Version desc) as rowNum, * from SanjelDataUAT.dbo.BlendFluidType) a
    where 
        rowNum = 1
)
,es_b_extra as 
...
(
    select
        ...
        --, owner_id
    from 
        (select ROW_NUMBER() OVER (Partition By Id ORDER BY Version desc) as rowNum, * from SanjelDataUAT.dbo.AdditiveType) a
    where 
        rowNum = 1
)
,es_a_extra as 
...
-- Add new columns for eService/eProgram changes
